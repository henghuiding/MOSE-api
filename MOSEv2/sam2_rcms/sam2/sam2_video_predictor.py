...
class SAM2VideoPredictor(SAM2Base):
    ...
    def __init__(
        ...
        # number of previous frames to promote to conditioning memory when disappearance is detected
        num_prev_frames_to_promote=3,
        ...
        **kwargs,
        ...
        self.num_prev_frames_to_promote = num_prev_frames_to_promote
        ...
        for frame_idx in tqdm(processing_order, desc="propagate in video"):
            ...
            for obj_idx in range(batch_size):
                ...
                if frame_idx in obj_output_dict["cond_frame_outputs"]:
                    ...
                    if not obj_output_dict.get("_disappear_detected", False):
                        obj_logits = current_out.get("object_score_logits", None)
                        if obj_logits is not None:
                            is_disappearing = torch.max(obj_logits) <= 0
                            if is_disappearing:
                                obj_output_dict["_disappear_detected"] = True
                                obj_output_dict["_disappear_frame_idx"] = frame_idx
                                # Promote a configurable number of recent frames to conditioning memory
                                promoted = 0
                                k = 1
                                max_promote = int(getattr(self, "num_prev_frames_to_promote", 4))
                                while promoted < max_promote and k < frame_idx:
                                    prev_idx = frame_idx - k
                                    if prev_idx in obj_output_dict["non_cond_frame_outputs"]:
                                        prev_out = obj_output_dict["non_cond_frame_outputs"][prev_idx]
                                        if prev_out.get("quality", 1.0) > self.low_quality_threshold:
                                            obj_output_dict["cond_frame_outputs"][prev_idx] = obj_output_dict["non_cond_frame_outputs"].pop(prev_idx)
                                            promoted += 1
                                    k += 1
    ...
    def _run_single_frame_inference(
        ...
        mask_quality = pred_masks.sigmoid()[pred_masks > 0.].sum() / (pred_masks > 0.).sum()
        ...
        compact_current_out = {
            ...
            "quality": object_score_logits.sigmoid().item() * current_out["ious"].max().item() * mask_quality.item()
        }
