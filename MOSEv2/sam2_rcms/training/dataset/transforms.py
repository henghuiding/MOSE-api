...
class RandomCropAPI:
    def __init__(self, size=None, sizes=None, consistent_transform=True, pad_if_needed=False, prob=1.0):
        """
        Random crop for video datapoints with mask alignment.
        - size: int or (h, w). Backward-compatible single size
        - sizes: list of int or (h, w). If provided, randomly choose one per call (or per frame if not consistent)
        - consistent_transform: if True, sample one crop window and apply to all frames
        - pad_if_needed: if any frame is smaller than crop size, pad to make it valid
        - prob: probability to apply the crop; otherwise return datapoint unchanged
        """
        # normalize sizes
        self.sizes = None
        if sizes is not None:
            if isinstance(sizes, (int, tuple, list)) and not (
                isinstance(sizes, (list, tuple)) and len(sizes) > 0 and isinstance(sizes[0], (list, tuple, int))
            ):
                sizes = [sizes]
            assert len(sizes) > 0
            norm_sizes = []
            for s in sizes:
                if isinstance(s, int):
                    norm_sizes.append((s, s))
                else:
                    assert len(s) == 2
                    norm_sizes.append((int(s[0]), int(s[1])))
            self.sizes = norm_sizes
        else:
            if size is None:
                size = (512, 512)
    ...
    def _ensure_min_size(self, datapoint, index, th, tw):
        h, w = datapoint.frames[index].size
        pad_h = max(0, th - h)
        pad_w = max(0, tw - w)
        if self.pad_if_needed and (pad_h > 0 or pad_w > 0):
            # pad on the right and bottom
            return pad(datapoint, index, (pad_w, pad_h))
        return datapoint

    def _crop_frame(self, datapoint, index, top, left, th, tw):
        # crop image
        datapoint.frames[index].data = F.crop(datapoint.frames[index].data, top, left, th, tw)
        # crop masks
        for obj in datapoint.frames[index].objects:
            if obj.segment is not None:
                obj.segment = F.crop(obj.segment, top, left, th, tw)
        datapoint.frames[index].size = (th, tw)
        return datapoint
    ...
    def __call__(self, datapoint: VideoDatapoint, **kwargs):
        ...
        num_frames = len(datapoint.frames)

        # helper to choose a size
        def choose_size():
            if self.sizes is not None:
                th, tw = random.choice(self.sizes)
                return int(th), int(tw)
            return self.size

        if self.consistent_transform:
            th, tw = choose_size()
            # Optionally pad each frame to be at least (th, tw)
            if self.pad_if_needed:
                for i in range(num_frames):
                    datapoint = self._ensure_min_size(datapoint, i, th, tw)

            # compute a crop window valid for all frames
            heights = []
            widths = []
            for i in range(num_frames):
                ...
                heights.append(h)
                widths.append(w)
            H_common = min(heights)
            W_common = min(widths)
            if H_common < th or W_common < tw:
                if not self.pad_if_needed:
                    return datapoint
                H_common = max(H_common, th)
                W_common = max(W_common, tw)
            max_top = H_common - th
            max_left = W_common - tw
            top = 0 if max_top == 0 else random.randint(0, max_top)
            left = 0 if max_left == 0 else random.randint(0, max_left)
            for i in range(num_frames):
                datapoint = self._crop_frame(datapoint, i, top, left, th, tw)
            return datapoint

        # not consistent: independent random crop per frame, possibly with different sizes
        for i in range(num_frames):
            th, tw = choose_size()
            h, w = datapoint.frames[i].size
            if h < th or w < tw:
                if not self.pad_if_needed:
                    # skip if invalid and cannot pad
                    continue
                datapoint = self._ensure_min_size(datapoint, i, th, tw)
                h, w = datapoint.frames[i].size
            max_top = h - th
