...
class EMAConf:
    enabled: bool = False
    decay: float = 0.9999
    update_every: int = 1  # Update EMA every N steps
...
class Trainer:
    ...
    def __init__(
        ...
        *,  # the order of these args can change at any time, so they are keyword-only
        ...
        self.loss_conf = loss
        self.ema_conf = EMAConf(**ema) if ema is not None else EMAConf()
    ...
    def _move_to_device(self):
        ...
        # Move EMA model to device if enabled
        if self.ema_conf.enabled and self.ema_model is not None:
            self.ema_model.to(self.device)
            logging.info(f"Moved EMA model to device {self.device}")
    ...
    def save_checkpoint(self, epoch, checkpoint_names=None):
        ...
        # Save regular checkpoint (always)
        ...
        if self.ema_conf.enabled and self.ema_model is not None:
            ema_state_dict = unwrap_ddp_if_wrapped(self.ema_model).state_dict()
            ema_state_dict = exclude_params_matching_unix_pattern(
                patterns=self.checkpoint_conf.skip_saving_parameters, state_dict=ema_state_dict
            )
            
            ema_checkpoint = {
                "model": ema_state_dict,  # Use 'model' key for consistency
                "optimizer": self.optim.optimizer.state_dict(),
                "epoch": epoch,
                "loss": self.loss.state_dict(),
                "steps": self.steps,
                "time_elapsed": self.time_elapsed_meter.val,
                "best_meter_values": self.best_meter_values,
                "ema_conf": self.ema_conf.__dict__,
                "is_ema_checkpoint": True,  # Flag to identify EMA checkpoint
            }
            
            if self.optim_conf.amp.enabled:
                ema_checkpoint["scaler"] = self.scaler.state_dict()
            
            # Save EMA checkpoint with '_ema' suffix
            if self.distributed_rank == 0:
                for ckpt_name in checkpoint_names:
                    ema_checkpoint_path = os.path.join(checkpoint_folder, f"{ckpt_name}_ema.pt")
                    self._save_checkpoint(ema_checkpoint, ema_checkpoint_path)
                logging.info(f"Saved EMA checkpoint for epoch {epoch}")
    ...
    def _load_resuming_checkpoint(self, ckpt_path: str):
        ...
        # Load EMA model if it exists in checkpoint
        if "ema_model" in checkpoint and self.ema_conf.enabled and self.ema_model is not None:
            load_state_dict_into_model(
                model=self.ema_model,
                state_dict=checkpoint["ema_model"],
                ignore_missing_keys=self.checkpoint_conf.skip_saving_parameters,
            )
            logging.info("Loaded EMA model from checkpoint")
        elif "is_ema_checkpoint" in checkpoint and checkpoint["is_ema_checkpoint"]:
            # This is an EMA checkpoint, load it into EMA model
            if self.ema_conf.enabled and self.ema_model is not None:
                load_state_dict_into_model(
                    model=self.ema_model,
                    state_dict=checkpoint["model"],
                ...
                logging.info("Loaded EMA model from EMA checkpoint")
            else:
                logging.warning("Found EMA checkpoint but EMA is not enabled in current config")
    ...
    def train_epoch(self, train_loader):
        ...
        for data_iter, batch in enumerate(train_loader):
            ...
            try:
                ...
                if self.ema_conf.enabled and self.steps[phase] % self.ema_conf.update_every == 0:
                    self._update_ema()
    ...
    def _setup_components(self):
        ...
        self.model = instantiate(self.model_conf)
        ...
        if self.ema_conf.enabled:
            ...
            logging.info(f"EMA enabled with decay={self.ema_conf.decay}, update_every={self.ema_conf.update_every}")
    ...
    def _update_ema(self):
        ...
        if not self.ema_conf.enabled or self.ema_model is None:
            return
        
        with torch.no_grad():
            for param, ema_param in zip(self.model.parameters(), self.ema_model.parameters()):
                ema_param.data.copy_(self.ema_conf.decay * ema_param.data + (1. - self.ema_conf.decay) * param.data)
